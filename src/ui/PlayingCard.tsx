...
const SIZES = {
  sm: 'playing-card-sm h-12 w-9 text-sm rounded-md',
  md: 'playing-card-md h-16 w-12 text-base rounded-lg',
  lg: 'playing-card-lg h-24 w-[4.5rem] text-2xl rounded-xl',
};
...
export function PlayingCard({ card, faceDown, size = 'md', dealt, className = '' }: Props) {
  ...
  if (faceDown || !card) {
    return (
      <div
        className={`playing-card ${sizeClass} ${dealt ? 'animate-dealIn' : ''} flex items-center justify-center border border-blue-900/60 bg-gradient-to-br from-blue-700 to-blue-900 shadow-md ${className}`}
        aria-label="暗牌"
      >
  ...
  return (
    <div
      className={`playing-card ${sizeClass} ${dealt ? 'animate-flipIn' : ''} relative flex flex-col justify-between bg-white p-1 font-semibold shadow-lg ring-1 ring-black/10 ${className}`}
